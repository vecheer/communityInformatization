    ...
    <div class="row">
        ...
        <div class="col-sm-10">
            ...
            <div class="col-sm-10" style="margin-bottom: 12px">
                <form class="form-horizontal" id="amInfo">
                    <div class="form-group">
                        <label for="Cename" class="col-sm-2 control-label">活动主办方</label>
                        <div class="col-sm-4">
                            <input type="text" name="cename" class="form-control" id="Cename" value="${param.cename==null?"":param.cename}" placeholder="请输入将删除的活动名称">
                        </div>
                    </div>
                    <div class="form-group">
                        <label for="Cesponsor" class="col-sm-2 control-label">活动主办方</label>
                        <div class="col-sm-4">
                            <input type="text" name="cesponsor" class="form-control" id="Cesponsor" value="${param.cesponsor==null?"":param.cesponsor}" placeholder="请输入将删除活动的举办方">
                        </div>
...
<script type="text/javascript">
    function deleteThis() {
        ...
        var cesponsor = $("#Cesponsor").val();

        if (cename==null || cename=="" || cename==undefined || cesponsor==null || cesponsor=="" || cesponsor==undefined ) {
            $("#msg").html("<h3 style=\"color:orangered\">请输入活动名称和活动的主办方</h3>");
            return;
        }

        var jsonstr = '{"cename":"'+ cename +'","cesponsor":"' + cesponsor + '"}';

        jQuery.support.cors = true;
        $.ajax({
            type: "post",
            url: "${pageContext.request.contextPath}/communistEvent/deleteCommunistEvent",
            data: jsonstr,
            dataType: "json",
            contentType: "application/json;charset=utf-8",
            success: function (data) {
                if (data.msg=="success"){
                    $("#msg").html("<h3 style=\"color:lightgreen\">提示: 该活动的信息已成功删除!</h3>");
                }else{
                    $("#msg").html("<h3 style=\"color:orangered\">提示: 未找到该活动，请检查活动名称和主办方是否错误</h3>");
                }
            },
            error: function (xhr) {
                alert("出错了!succes未执行");
