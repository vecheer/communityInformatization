            ...
            <div class="col-sm-10" style="margin-bottom: 12px">
                <form class="form-horizontal" id="amInfo">
                    <div class="form-group">
                        <label for="Aidnum" class="col-sm-2 control-label">党员身份证号</label>
                        <div class="col-sm-4">
                            <input type="text" name="aidnum" class="form-control" id="Aidnum" value="${param.aidnum==null?"":param.aidnum}" placeholder="请输入将删除的积极分子身份证号">
                        </div>
...
<script type="text/javascript">
    function deleteThis() {
        ...
        var aidnum = $("#Aidnum").val();
        if (aidnum==null || aidnum=="" || aidnum==undefined) {
            $("#msg").html("<h2>请输入正确的积极分子身份证号码!</h2>");
            return;
        }

        var jsonstr = '{"aidnum":'+ aidnum +'}';

        jQuery.support.cors = true;
        $.ajax({
            type: "post",
            url: "${pageContext.request.contextPath}/activeMember/deleteActiveMember",
            data: jsonstr,
            dataType: "json",
            contentType: "application/json;charset=utf-8",
            success: function (data) {
                if (data.msg=="success"){
                    $("#msg").html("<h3 style=\"color:lightgreen\">提示: 该积极分子的信息已成功删除!</h3>");
                }else{
                    $("#msg").html("<h3 style=\"color:orangered\">提示: 未找到该积极分子，请检查身份证号是否正确</h3>");
                }
            },
            error: function (xhr) {
                alert("出错了!succes未执行");
