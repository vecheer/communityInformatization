        ...
        <div class="col-sm-10">
            ...
            <div class="col-md-10" style="margin-top: 20px">
                ...
                <div class="row-fluid" align="left">
                    <div style="float: left; margin-top: 2px; text-align: center;">
                        ...
                        <%
                            ...
                            if (request.getAttribute("ajoinbranch") != null)
                                ajoinbranch = (String) request.getAttribute("ajoinbranch");

                        %>
                        <form name="query_form"
                              action="${pageContext.request.contextPath}/activeMember/findActiveMemberByBranch" method="post">
                            <input type="text"  name="ajoinbranch" value="<%= ajoinbranch %>" placeholder="请输入培养积极分子的支部名称"/>
                ...
                <div class="col-sm-12">
                    ...
                    <%
                        //判断是否有提示信息需要显示
                        if (request.getAttribute("activeMembers") != null) {
                            if (((List<ActiveMember>) request.getAttribute("activeMembers")).size() == 0) {
                    %>
                    ...
                    <div id="result_table" class="table-responsive" style="overflow:auto;border: 1px">
                        <table class="table table-hover" style="text-align: center">
                            <caption style="font-size: 1.5em;text-align: left">积极分子</caption>
                            ...
                            <%
                                ArrayList<ActiveMember> activeMembers = (ArrayList<ActiveMember>) request.getAttribute("activeMembers");
                                for (ActiveMember a : activeMembers) {
                            %>
                            <tr>
                                <td><a href="${pageContext.request.contextPath}/func/PartyBuilding/func_amInfo_update.jsp?aid=<%= a.getAid() %>&
                                &aname=<%= a.getAname() %>&
                                &asex=<%= a.getAsex() %>&
                                &anationality=<%= a.getAnationality() %>&
                                &aidnum=<%= a.getAidnum() %>&
                                &aedulevel=<%= a.getAedulevel() %>&
                                &aworkentrytime=<%= a.getAworkentrytime() %>&
                                &aworkat=<%= a.getAworkat() %>&
                                &aduty=<%= a.getAduty() %>&
                                &aaddress=<%= a.getAaddress() %>&
                                &aleagueentrydate=<%= a.getAleagueentrydate() %>&
                                &aapplydate=<%= a.getAapplydate() %>&
                                &abeamdate=<%= a.getAbeamdate() %>&
                                &areferrer=<%= a.getAreferrer() %>&
                                &ajoinbranch=<%= a.getAjoinbranch() %>" style="text-decoration: none">修改</a>
                                </td>

                                <td><a href="${pageContext.request.contextPath}/func/PartyBuilding/func_amInfo_delete.jsp?aidnum=<%= a.getAidnum()%>"
                                       style="text-decoration: none" >删除</a>
                                </td>
                                <td><%= a.getAid() %></td>
                                <td><%= a.getAname() %></td>
                                <td><%= a.getAsex() %></td>
                                <td><%= a.getAnationality() %></td>
                                <td><%= a.getAidnum() %></td>
                                <td><%= a.getAedulevel() %></td>
                                <td><%= a.getAworkentrytime() %></td>
                                <td><%= a.getAworkat() %></td>
                                <td><%= a.getAduty() %></td>
                                <td><%= a.getAaddress() %></td>
                                <td><%= a.getAleagueentrydate() %></td>
                                <td><%= a.getAapplydate() %></td>
                                <td><%= a.getAbeamdate() %></td>
                                <td><%= a.getAreferrer() %></td>
                                <td><%= a.getAjoinbranch() %></td>
                            </tr>
                            <%
                                }
                            %>
                ...
                <div class="row-fluid" style="float: right">
                    ...
                    %>
                    <form id="pageform" name="pageform" action="${pageContext.request.contextPath}/activeMember/findActiveMemberByBranch" method="post">
                        <!--直接提交本表单，本表单是隐藏的 -->
                        <input type="hidden" name="ajoinbranch" value="<%=ajoinbranch%>"/>
